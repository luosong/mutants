...
/***/ function(module, exports, __webpack_require__) {
	...
	var _VirtualList = __webpack_require__(574);

	var _VirtualList2 = _interopRequireDefault(_VirtualList);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	exports.default = _VirtualList2.default; /**
	                                          * Created by TonyJiang on 16/5/3.
	                                          */
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _VirtualListItemWrapper = __webpack_require__(575);

	var _VirtualListItemWrapper2 = _interopRequireDefault(_VirtualListItemWrapper);
	...
	var propTypes = _chanjetScroller.commonProps.propTypes;
	var defaultProps = _chanjetScroller.commonProps.defaultProps;
	...
	var VirtualList = function (_React$Component) {
	    ...
	    function VirtualList() {
	        ...
	        return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = VirtualList.__proto__ || Object.getPrototypeOf(VirtualList)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
	            syncHeight: true,
	            offsetTop: 0,
	            iosMainVersion: 0,
	            outerUpdateId: Date.now(),
	            showSize: 10
	            ...
	            redundancyFrom: 0,
	            redundancyTo: 10
	        }, _this._heightCache = [], _temp), _possibleConstructorReturn(_this, _ret);
	    }
	    ...
	    _createClass(VirtualList, [{
	        ...
	        value: function componentWillMount() {
	            ...
	            var redundancy = _props.redundancy;


	            this.state.size = size;
	            this.state.redundancy = redundancy;
	            this.state.showSize = this._position.to + redundancy;

	            this._initHeightCache();

	            // if(mutants.env.os == mutants.env.constant.os.ios){
	            //     var version = navigator.userAgent.match(/OS ([\d]*_\d[_\d]*) like Mac OS X/i)[1];
	            //     this.state.iosMainVersion = version.split('_')[0];
	            // }
	        }
	    }, {
	        key: 'componentDidMount',
	         ...
	         * 如果数据长度变短 : 则从尾部减少
	         *
	         * 如果数据长度变短,需要判断当前是否显示到底, 如果是则需要调整显示的索引号
	         */

	    }, {
	        key: 'componentWillReceiveProps',
	        value: function componentWillReceiveProps(props) {
	            this.state.outerUpdateId = Date.now();

	            var currentSize = this.state.size;
	            this.state.size = props.size;

	            if (props.size > currentSize) {
	                this._appendHeightCache();
	            } else if (props.size < currentSize) {
	                // this.state.size = props.size;
	                this._reduceHeightCache();
	            }

	            if (this._position.to > props.size - 1) {
	                var reduce = props.size - this._position.to;

	                this._position.from += reduce;
	                this._position.from = Math.max(0, this._position.from);
	                this._position.to += reduce;
	                this._position.to = Math.max(0, this._position.to);
	            }
	        ...
	        key: '_initHeightCache',
	        value: function _initHeightCache() {
	            this._heightCache.length = 0;
	            for (var i = 0; i < this.state.size; i++) {
	                this._heightCache.push({
	                    height: this.props.estimateHeight,
	                    top: i * this.props.estimateHeight,
	                    hasRender: false
	                });
	            }
	        }
	    }, {
	        key: '_appendHeightCache',
	        value: function _appendHeightCache() {
	            var length = this.state.size - this._heightCache.length;
	            var lastItem = this._heightCache[this._heightCache.length - 1];
	            var startPosition = lastItem.top + lastItem.height;
	            for (var i = 0; i < length; i++) {
	                this._heightCache.push({
	                    height: this.props.estimateHeight,
	                    top: startPosition + i * this.props.estimateHeight,
	                    hasRender: false
	                });
	            }
	        }
	    }, {
	        key: '_reduceHeightCache',
	        value: function _reduceHeightCache() {
	            // const length = this._heightCache.length - this.state.size;
	            // console.log('reduceHeight' , this._heightCache.length , this.state.size);
	            this._heightCache.length = this.state.size;
	        }
	        ...
	        key: '_updateHeightCache',
	        value: function _updateHeightCache() {
	            var dom = this.refs.listWrapper;
	            var redundancyFrom = this._position.redundancyFrom;


	            var sumHeight = this.state.offsetTop;
	            var needResetTop = false;

	            //重新计算当前所有子元素的高度
	            for (var index = 0; index < dom.children.length; index++) {

	                // const style = window.getComputedStyle(dom.children[index]);
	                // const height = parseInt(style.height) + parseInt(style.marginTop) + parseInt(style.marginBottom);

	                //offsetHeight , 不进行getComputedStyle
	                var currentIndex = redundancyFrom + index;
	                var height = dom.children[index].offsetHeight || this._heightCache[currentIndex].height;

	                if (this._heightCache[currentIndex].height != height) {
	                    needResetTop = true;
	                }
	                this._heightCache[currentIndex].height = height;
	                this._heightCache[currentIndex].hasRender = true;
	            ...
	            if (needResetTop) {
	                //更新后续元素的top
	                for (var i = 1; i < this._heightCache.length; i++) {
	                    this._heightCache[i].top = this._heightCache[i - 1].top + this._heightCache[i - 1].height;
	                }
	            }

	            if (this._heightCache.length > 0) {
	                var _lastOne = this._heightCache[this._heightCache.length - 1];

	                this.state.totalHeight = _lastOne.top + _lastOne.height;
	            } else {
	                this.state.totalHeight = 0;
	            }

	            this.refs.listHeightWrapper.style.height = this.state.totalHeight + 'px';
	        }

	        /**
	         * 根据显示数据索引{from,to} 显示数据 , 并且对ItemWrapper的key进行管理
	         */
	        ...
	        value: function render() {
	            ...
	            var from = _position.from;
	            var to = _position.to;

	            var other = _objectWithoutProperties(this.props, []);

	            // const redundancyFrom = Math.max(from - this.state.redundancy, 0);
	            // const regit pudundancyTo = Math.min(to + this.state.redundancy, this.state.size);

	            var _calculateRedundancyP = this._calculateRedundancyPosition(from, to);

	            var redundancyFrom = _calculateRedundancyP.redundancyFrom;
	            var redundancyTo = _calculateRedundancyP.redundancyTo;


	            this._position.redundancyFrom = redundancyFrom;
	            this._position.redundancyTo = redundancyTo;

	            // console.log('from and to' , from ,to);
	            // console.log('redundancyFrom and To' , redundancyFrom , redundancyTo);
	            //当前显示的区域大小
	            // this.state.showSize = Math.max(redundancyTo - redundancyFrom, this.state.showSize);

	            var items = [];

	            // console.debug('渲染第 ' + redundancyFrom + ' 到 ' + redundancyTo + ' 个元素');

	            //key=i%showSize , 实现了从隐藏方向抽取并添加到显示方向上,避免在滚动过程中整体redraw
	            for (var i = redundancyFrom, key = 0; i < redundancyTo; i++, key++) {
	                var _key2 = i % this.state.showSize;

	                // items.push(this.props.itemRenderer(i , i%showSize));
	                items.push(_react2.default.createElement(_VirtualListItemWrapper2.default, {
	                    itemRenderer: this.props.itemRenderer,
	                    dataIndex: i,
	                    reactKey: _key2.toString(),
	                    key: _key2.toString(),
	                    heightCache: this._heightCache[i],
	                    outerUpdateId: this.state.outerUpdateId
	                }));
	            }

	            //设置偏移位置
	            this.state.offsetTop = redundancyFrom != redundancyTo ? this._heightCache[redundancyFrom].top : 0;

	            var listWrapperStyle = {
	                transform: 'translate3d(0px , ' + this.state.offsetTop + 'px , 0px)',
	                WebkitTransform: 'translate3d(0px , ' + this.state.offsetTop + 'px , 0px)'
	            };
	            ...
	            return _react2.default.createElement(
	                ...
	                _extends({
	                    ...
	                    refreshHandler: this.props.refreshHandler ? this._refreshHandler.bind(this) : null,
	                    moreHandler: this.props.moreHandler ? this._moreHandler.bind(this) : null

	                }),
	                _react2.default.createElement(
	                    'div',
	                    { ref: 'listHeightWrapper' },
	                    _react2.default.createElement(
	                        'div',
	                        { ref: 'listWrapper', style: listWrapperStyle },
	                        items
	                    )
	                )
	            );
	        ...
	        key: '_calculateRedundancyPosition',
	        value: function _calculateRedundancyPosition(from, to) {
	            ...
	            //冗余
	            var redundancy = this.state.showSize - (to - from);

	            var halfRedundancy = Math.ceil(redundancy / 2);
	            //总数
	            var size = this.state.size;

	            //如果头部不够等分插入,则往尾部追加

	            if (from < halfRedundancy) {

	                position.redundancyFrom = 0;
	                position.redundancyTo = Math.min(to + redundancy - from, size);

	                //如果尾部不够等分插入,则往头部追加
	            } else if (size - to < halfRedundancy) {

	                position.redundancyTo = size;
	                position.redundancyFrom = Math.max(from - (redundancy - (position.redundancyTo - to)), 0);

	                //等分插入
	            } else {
	                position.redundancyFrom = from - halfRedundancy;
	                position.redundancyTo = to + redundancy - (from - position.redundancyFrom);
	            }

	            return position;
	        }

	        //滚动监听 , 根据滚动位置计算当前需要显示的数据
	        //根当前的顶部元素进行对比 , 重新计算出新的from和to
	        ...
	        value: function _scrollHandler(e) {
	            ...
	            //新版本scroller的scrollTop支持负数, 所以要保证这里是正数

	            var scrollTop = Math.max(0, e.scrollTop);

	            this.state.scrollTop = scrollTop;
	            this.state.offsetHeight = offsetHeight;

	            var _position2 = this._position;
	            var from = _position2.from;
	            var to = _position2.to;

	            var startElement = this._heightCache[from];

	            var direction = undefined;

	            var startTop = startElement.top;
	            var startBottom = startElement.top + startElement.height;

	            //1.首先确认滚动方向
	            if (scrollTop > startBottom) {
	                direction = 'down';
	            ...
	            //2.重新确定startElement
	            var newFrom = from;
	            var _startElementTop = void 0;
	            var _startElementBottom = void 0;
	            var increase = void 0;
	            if (direction == 'down') {
	                increase = 1;
	            } else {
	            ...
	            for (var len = this._heightCache.length; newFrom < len; newFrom = newFrom + increase) {
	                _startElementTop = this._heightCache[newFrom].top;
	                _startElementBottom = this._heightCache[newFrom].top + this._heightCache[newFrom].height;
	                // console.log('scroll is ' , scrollTop , 'newFrom is ' , newFrom , 'top&bottom is ' , _startElementTop , _startElementBottom)
	                if (_startElementTop <= scrollTop && _startElementBottom >= scrollTop) {
	                    break;
	                }
	            }
	            ...
	            //3.重新确定toElement
	            var newTo = newFrom;
	            var toElement = this._heightCache[newFrom];
	            var _toElementTop = void 0;
	            var _toElementBottom = void 0;
	            for (var len = this._heightCache.length; newTo < len; newTo++) {
	                _toElementTop = this._heightCache[newTo].top;
	                _toElementBottom = this._heightCache[newTo].top + this._heightCache[newTo].height;
	                if (_toElementTop > scrollTop + offsetHeight) {
	                    // console.log('screenBottom is ' , scrollTop + offsetHeight , 'newTo is ', newTo , 'toElement is' , _toElementTop);
	                    break;
	                }
	            }

	            //4.增加冗余 , 控制不超过边界 [0 , this_heightCache.length]
	            // newFrom = Math.max(newFrom - this.state.redundancy , 0);
	            // newTo = Math.min(newTo + this.state.redundancy , this.state.size);

	            if (newFrom != this._position.from || newTo != this._position.to) {

	                this._position.from = newFrom;
	                this._position.to = newTo;
	        ...
	        key: '_refreshHandler',
	        value: function _refreshHandler(done) {
	            var _this2 = this;

	            if (this.props.refreshHandler) {
	                this.props.refreshHandler(function (newSize) {
	                    _this2.state.size = newSize;
	                    //刷新需要重置高度缓存
	                    _this2._initHeightCache();
	                    //更新outerUpdateId, 保证当前显示的item强制render
	                    _this2.state.outerUpdateId = Date.now();
	                    _this2.forceUpdate();
	                    done();
	                });
	            }
	        ...
	        key: '_moreHandler',
	        value: function _moreHandler(done) {
	            var _this3 = this;

	            if (this.props.moreHandler) {
	                this.props.moreHandler(function (newSize) {
	                    _this3.state.size = newSize;
	                    //追加元素需要追加高度缓存
	                    _this3._appendHeightCache();
	                    //更新outerUpdateId, 保证当前显示的item强制render
	                    _this3.state.outerUpdateId = Date.now();
	                    _this3.forceUpdate();
	                    done();
	                });
	            }
	...
	VirtualList.propTypes = Object.assign({}, propTypes, {
	    size: _react2.default.PropTypes.number.isRequired,
	    itemRenderer: _react2.default.PropTypes.func.isRequired,
	    estimateHeight: _react2.default.PropTypes.number.isRequired,
	    redundancy: _react2.default.PropTypes.number
	});
	VirtualList.defaultProps = Object.assign({
	    redundancy: 10
	}, defaultProps);
...
/***/ function(module, exports, __webpack_require__) {
	...
	function _inherits(subClass, superClass) { if (typeof superClass !== "function" && superClass !== null) { throw new TypeError("Super expression must either be null or a function, not " + typeof superClass); } subClass.prototype = Object.create(superClass && superClass.prototype, { constructor: { value: subClass, enumerable: false, writable: true, configurable: true } }); if (superClass) Object.setPrototypeOf ? Object.setPrototypeOf(subClass, superClass) : subClass.__proto__ = superClass; } /**
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                * item包装器, 用于进行React渲染优化使用, 不会对item的dom进行注入.
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                * 保证只有头部或者尾部节点进行插入重用时,才会触发render
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                */


	var VirtualListItemWrapper = function (_React$Component) {
	    _inherits(VirtualListItemWrapper, _React$Component);

	    function VirtualListItemWrapper() {
	        _classCallCheck(this, VirtualListItemWrapper);

	        return _possibleConstructorReturn(this, (VirtualListItemWrapper.__proto__ || Object.getPrototypeOf(VirtualListItemWrapper)).apply(this, arguments));
	    }

	    _createClass(VirtualListItemWrapper, [{
	        key: 'shouldComponentUpdate',


	        //如果是外部刷新 , 或者当前组件被重用的时候 , 才会进行render
	        value: function shouldComponentUpdate(nextProps, nextStates) {
	            return nextProps.dataIndex !== this.props.dataIndex || nextProps.outerUpdateId !== this.props.outerUpdateId;
	        }
	    }, {
	        key: 'render',
	        value: function render() {
	            // console.log('item.render :', this.props.dataIndex)

	            var _props = this.props;
	            var dataIndex = _props.dataIndex;
	            var reactKey = _props.reactKey;

	            return this.props.itemRenderer(dataIndex, reactKey);
	        }
	    }]);

	    return VirtualListItemWrapper;
	}(_react2.default.Component);

	VirtualListItemWrapper.propTypes = {
	    reactKey: _react2.default.PropTypes.string.isRequired,
	    dataIndex: _react2.default.PropTypes.number.isRequired,
	    itemRenderer: _react2.default.PropTypes.func.isRequired,
	    outerUpdateId: _react2.default.PropTypes.number.isRequired,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _chanjetVirtualList = __webpack_require__(573);

	var _chanjetVirtualList2 = _interopRequireDefault(_chanjetVirtualList);
	...
	var _FakerAvatarImages = __webpack_require__(576);
	...
	var IndexPage = function (_BasePage) {
	    ...
	    _createClass(IndexPage, [{
	        ...
	        value: function initData() {
	            ...
	            for (var i = 0; i < 2000; i++) {
	                this.data.push({
	                    ...
	                    //随机生成padding高度, 高度在 0 ~ 60 范围内
	                    padding: faker.random.number({ min: 0, max: 60 }),
	        ...
	        value: function getStyles() {
	            ...
	            return {
	                //定义显示区域大小
	                wrapper: { width: '100%', height: '100%', overflowY: 'hidden' },
	                //定义每一个元素大小
	                item: { borderBottom: "0.02666666666666667rem solid #e5e5e5", userSelect: 'none', WebkitUserSelect: 'none' }
	            };
	        }
	    }, {
	        key: "itemRenderer",
	        value: function itemRenderer(index, key) {
	            var styles = this.getStyles();
	            var data = this.data[index];

	            //设置当前item的style
	            var itemStyle = Object.assign({}, styles.item, {
	                padding: data.padding + "px 0"
	            });
	        ...
	        value: function renderContent() {
	            ...
	            return _react2.default.createElement(
	                ...
	                _react2.default.createElement(_chanjetVirtualList2.default, {
